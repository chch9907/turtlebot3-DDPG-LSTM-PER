...
class Env():
    def __init__(self, action_size):
        ...
        self.img_rows = 32
        self.img_cols = 32
        self.img_channels = 4 # 4 frames for 1 state
        self.jump = 0
    ...
    # def smooth_velocity(self, x, y): #dont forget add "self " as a parameter!!
    #     lx = np.sqrt(x.dot(x))
    #     ly = np.sqrt(y.dot(y))
    #     cos_angle = x.dot(y) / (lx * ly)
    #     angle = np.arccos(cos_angle)  # pi 
    ...
    def step(self, action):
        ...
        # self. cur_pos = np.array([round(self.position.x,2),round(self.position.y,2)])
        
        success=False
        cv_image = None
        state_img = []
        for i in range(4): # 1 state consists of 4 frames 
            image_data = None
            ...
            cv_image = cv_image.reshape(1, cv_image.shape[0], cv_image.shape[1])
            state_img.append(cv_image)
        
        state_img =np.array(state_img).reshape(4, 32, 32)
        ...
        #rospy.loginfo("Calculate the reward ") 
    ...
    def reset(self):
        ...
        #reset the pre_position and cur_position
        # self.cur_pos = np.array([round(self.position.x,2), round(self.position.y,2)])
        # self.pre_pos = np.zeros(2)
        # self.pre_orient = np.zeros(2)
        # self.cur_orient = np.zeros(2)
        ...
        for i in range(4): # 1 state consists of 4 frames 
            ...
            while image_data is None or success is False:
                try:
                    #rospy.loginfo("reset: try camera data")
                    image_data = rospy.wait_for_message('/camera/rgb/image_raw', Image, timeout=5)
                    h = image_data.height
                    w = image_data.width
                    cv_image = CvBridge().imgmsg_to_cv2(image_data, "bgr8")
                    #rospy.loginfo("reset: Capture the image")
                    #temporal fix, check image is not corrupted
                    if not (cv_image[h//2,w//2,0]==178 and cv_image[h//2,w//2,1]==178 and cv_image[h//2,w//2,2]==178):
                        success = True
                    else:
            ...
            #cv_image = cv_image[(self.img_rows/20):self.img_rows-(self.img_rows/20),(self.img_cols/10):self.img_cols] #crop image
            #cv_image = skimage.exposure.rescale_intensity(cv_image,out_range=(0,255))
            cv_image = cv_image.reshape(1, cv_image.shape[0], cv_image.shape[1])#(1 batchsize, )1 channel, rows, cols
            state_img.append(cv_image)
        state_img =np.array(state_img).reshape(4, 32, 32)
        cur_dis = self.getGoalDistance()
        goal_angle = math.atan2(self.goal_y - self.position.y, self.goal_x - self.position.x)
